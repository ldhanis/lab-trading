...
def user_register(request):
	...
	if request.method == 'POST':
		...
		if User.objects.filter(email__iexact=request.POST.get('email')).first():
			messages.warning(request, _("Cette addresse email est déjà utilisée"))
			...
			messages.warning(request, _("Une erreur s'est produite lors de votre inscription, veuillez réessayer"))
